...
from utils.running_utils import get_gpu_device, initlogging, resetRandomStates, readConfigFile
...
if __name__ == "__main__":
    ...
    for mode in ("reg", "cla"):    # two-stage training
        ...
        logging.critical(f"\nProcessing mode = [{mode}] for layer = [{layer}] in LLM = [{modeldir}] and dataset = [{datasetdir}]")
        print()
        ...
        logging.critical(f"Reading train data from {traindatapath}")
        wwset = WordbyWordDataset(traindatapath, EOSID)
        
        testdatapath = os.path.join(fulllayerfolder, f"{modeldir}-{datasetdir}-layer{layer}-test-data") 
        ...
        if mode == "cla":
            if scratch:
                ...
                pretrainedModelPath = os.path.join(fulllayerfolder, f"{modeldir}-{datasetdir}_Train-MLPRegressor-Scratch-reg-layer{layer}.mdl")
        ...
        suffix = f"-{mlpModel.__class__.__name__}-{scratchstr}-{mode}-layer{layer}"    
        ...
        logging.critical(f"Save attack model to {modelsavepath}\n")
